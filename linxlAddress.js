...
(function(global, factory) {
	...
	const linxlAddress = function(mixinStr) {
		...
		let _province = '',
			...
			countyList = [],
			cityList = [];
		let familyName = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '楮', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤',
			...
			'索', '咸', '籍', '赖', '卓', '蔺', '屠', '蒙', '池', '乔', '阴', '郁', '胥', '能', '苍', '双', '闻', '莘', '党', '翟', '谭', '贡', '劳',
			...
			'乜', '养', '鞠', '须', '丰', '巢', '关', '蒯', '相', '查', '后', '荆', '红', '游', '竺', '权', '逑', '盖', '益', '桓', '公', '万俟',
			'司马', '上官', '欧阳', '夏侯', '诸葛', '闻人', '东方', '赫连', '皇甫', '尉迟', '公羊', '澹台', '公冶', '宗政', '濮阳', '淳于', '单于', '太叔', '申屠',
			'公孙', '仲孙', '轩辕', '令狐', '锺离', '宇文', '长孙', '慕容', '鲜于', '闾丘', '司徒', '司空', '丌官', '司寇', '仉', '督', '子车', '颛孙', '端木',
			'巫马', '公西', '漆雕', '乐正', '壤驷', '公良', '拓拔', '夹谷', '宰父', '谷梁', '晋', '楚', '阎', '法', '汝', '鄢', '涂', '钦', '段干', '百里',
			'东郭', '南门', '呼延', '归', '海', '羊舌', '微生', '岳', '帅', '缑', '亢', '况', '后', '有', '琴', '梁丘', '左丘', '东门', '西门', '商', '牟',
		...
		/** 获取省、市、区列表 **/
		provinceList = Object.keys(areaJson);
		Object.values(areaJson).forEach(item => {
			cityList.push(Object.keys(item))
		});
		cityList = cityList.toString().split(',');
		Object.values(areaJson).forEach(item => {
			countyList.push(Object.values(item))
		});
		countyList = countyList.toString().split(',');
		_phone = str.match(
			/((((13[0-9])|(15[^4])|(18[0,1,2,3,5-9])|(17[0-8])|(147))\d{8})|((\d3,4|\d{3,4}-|\s)?\d{7,14}))?/g);
		if (_phone) { //含有phone
			_phone = _phone.toString().replace(new RegExp(",", "g"), '');
			let other = str.split(_phone);
			if (other[0] && other[1]) { // phone在中间
				if (other[0].length > other[1].length) {
					_fullAddress = other[0];
					_name = other[1];
				} else {
					_fullAddress = other[1];
					_name = other[0];
				}
			} else { // phone在两边
				let tempNameAddress = other[0] || other[1];
				_name = getName(provinceList, tempNameAddress) || getName(cityList, tempNameAddress) || getName(countyList,
					tempNameAddress)
				let tempAddress = tempNameAddress.split(_name);
				_fullAddress = tempAddress[0] || tempAddress[1];
			}
		}
		...
		if (_fullAddress) { // 地址
			...
			if (_fullAddress.includes('区')) {
				...
				_address = _fullAddress.substring(tempIndex + 1, _fullAddress.length)
			} else {
				let cutCounty = _county.substring(0, _county.length - 1);
				tempIndex = _fullAddress.lastIndexOf(cutCounty) + cutCounty.length;
				_address = _fullAddress.substring(tempIndex, _fullAddress.length)
			}
			if (_county && _city) {
				for (let item in areaJson) {
			...
			if (_county && !_city) {
				for (let item in areaJson) {
					for (let v in areaJson[item]) {
						if (areaJson[item][v].includes(_county)) {
							_province = item;
							_city = v;
						}
					}
			...
			if (_city && !_province) {
				for (let item in areaJson) {
					if (areaJson[item][_city]) {
						_province = item;
					}
				}
			}
			_fullAddress = _province + _city + _county + _address;
			_region = [_province, _city, _county];
		}

		function getName(list, nameAddress) {
			let resultName = '';
			if (nameAddress) {
				list.forEach(item => {
					if (nameAddress.includes(item.substring(0, 2))) {
						let temp = item.substring(0, 2);
						let addressIndex = nameAddress.lastIndexOf(temp);
						if (addressIndex !== 0) { // 名字在左边
							resultName = nameAddress.substring(0, addressIndex);
						}
					}
				})
			}
		...
		let addressStr = _province + _city + _county;

		// 名字在右边
		if (addressStr.includes(_name)) {
			let nameIndexList = [];
			familyName.forEach(item => {
				if (_address.includes(item)) {
					nameIndexList.push(_address.lastIndexOf(item));
				}
			})
			let nameIndex = nameIndexList[0];
			_name = _address.substring(nameIndex, _address.length);
			_address = _address.substring(0, nameIndex);
		}
